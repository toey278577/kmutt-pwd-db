...
### ฟีเจอร์ใหม่ / ปรับปรุงรายงาน
- **Export รายงานเป็น Excel (.xlsx)** — เพิ่มปุ่ม "Excel" ในหน้าออกรายงาน สร้างไฟล์ Excel จริงด้วย SheetJS (`xlsx`)
  - แบบประเมินรายวิชา: หัวตารางคะแนน 5 หัวข้อ + ช่องรวม 25 คะแนน เว้นว่างให้กรอกใน Excel, มีแถวหัวเรื่อง (รายวิชา/วันที่/อาจารย์)
  - รายชื่อคนพิการ / แบบสังเกตพฤติกรรม: export ตารางรายชื่อพื้นฐาน (ชื่อ/อายุ/วันเกิด/เลขบัตร/ความพิการ/จังหวัด/เบอร์)
  - ตั้งความกว้างคอลัมน์ให้พอดี + ตั้งชื่อไฟล์ตามวันที่อัตโนมัติ
- **ปรับตารางแบบประเมินรายวิชา** — หดช่อง "ชื่อ - สกุล" (`min-w-28` → `w-24`) ที่กว้างเกินไป และขยายช่องคะแนนทุกช่อง (`w-14` → `w-16`) เขียนตัวเลขได้สบายขึ้น

### คำสั่งที่ใช้
```bash
cd frontend
npm install xlsx        # SheetJS สำหรับสร้างไฟล์ Excel
npm run build           # ทดสอบ build ผ่าน
git add -A && git commit -F <msg> && git push
```
